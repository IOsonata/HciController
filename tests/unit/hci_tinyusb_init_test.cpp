/*
 * Host test for TinyUSB bring up, retry semantics, full teardown and TX byte
 * preservation.
 */
...
extern "C" bool tusb_deinit(uint8_t rhport)
{
    ...
    if (rhport >= sizeof(gRhportRole) / sizeof(gRhportRole[0]) ||
        gRhportRole[rhport] != TUSB_ROLE_DEVICE)
    {
        return false;
    }

    gRhportRole[rhport] = TUSB_ROLE_INVALID;
    return true;
}
...
int main(void)
{
     ...
     * and this wrapper's singleton. Otherwise HciAppStop followed by HciAppInit
     * merely clears the C++ object while the USB stack still owns the port.
     */
    HciTinyUsbStop(&usb);
    assert(gDeinitCalls == 1U);
    assert(!usb.Started);
    assert(!HciTinyUsbIsMounted(&usb));
    assert(!tusb_inited());
    ...
    HciTinyUsb_t usb2;
    assert(HciTinyUsbInit(&usb2, &intrf, 0U, Wake, nullptr));
    assert(HciTinyUsbStart(&usb2));
    assert(gRhportInitCalls == 2U);
    assert(tusb_inited());
    HciTinyUsbStop(&usb2);
    assert(gDeinitCalls == 2U);
    assert(!tusb_inited());
    printf("[ok] full stop releases TinyUSB and singleton ownership\n");
