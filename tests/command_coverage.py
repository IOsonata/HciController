...
def validate_core54_source_contracts(root):
    ...
    if sca and cis and "HCI_SDC_COMPAT_OPCODE_LE_REQUEST_PEER_SCA" not in compat_source:
        raise SystemExit(
            "Core 5.4 C.44 is active (SCA Updates + CIS) but LE Request Peer "
            "SCA is not routed by the controller"
        )
    if sca and cis:
        print("[ok] C.44 active resource profile has LE Request Peer SCA routing")
