...
@brief	TinyUSB CDC H:4 transport and diagnostic CDC interface.

		Defines TinyUSB wrapper state, Host wake integration, buffered CDC
		processing, direct log writes, and USB port state queries.
...
/*
 * Deinitialize the device stack and release this wrapper's singleton
 * ownership. Call only after the target has disabled the USB peripheral and
 * its interrupt, so no callback can race the teardown. A stopped object can be
 * initialized again, and a different object can become the owner afterwards.
 */
