...
#define HCI_SDC_COMPAT_OPCODE_LE_REQUEST_PEER_SCA        0x206DU
...
/* -------------------------------------------------------------------------
 * Core HCI compatibility commands omitted from the local nrfxlib dispatch.
 * ------------------------------------------------------------------------- */
...
{
    /*
     * Core 5.4 Vol 4 Part E 7.8.27. The configured multirole product supports
     * every legacy combination in states 0..41: advertising and scanning are
     * enabled, parallel scanning+initiating is explicitly enabled, and the
     * resource profile reserves multiple Central and Peripheral links.
     *
     * The resource regression test locks those prerequisites independently;
     * the hardware Core 5.4 profile test verifies this exact wire value. Bits
     * 42..63 are reserved and remain zero.
     */
...
static HciCmdResult_t HciSdcCompatRequestPeerSca(void *,
                                                  const uint8_t *pParams,
                                                  size_t,
                                                  uint8_t *,
                                                  size_t)
{
    sdc_hci_cmd_le_request_peer_sca_t value;
    memcpy(&value, pParams, sizeof(value));

    /*
     * Core 5.4 7.8.108 is a long-running peer procedure: acknowledge it with
     * Command Status, then SDC supplies LE Request Peer SCA Complete from its
     * normal event queue.
     */
    HciCmdResult_t result = {
        sdc_hci_cmd_le_request_peer_sca(&value),
...
static const HciCmdEntry_t s_HciSdcCompatCommands[] = {
    ...
    {HCI_SDC_COMPAT_OPCODE_LE_REQUEST_PEER_SCA,
     2U,
     0U,
     HCI_CMD_RESPONSE_STATUS,
     HciSdcCompatRequestPeerSca},
};
 ...
 * Timeout to octet 7 bits 2 and 3, LE Read Supported States to octet 28 bit 3,
 * and LE Request Peer SCA to octet 43 bit 2.
 */
static void HciSdcPatchSupportedCommands(uint8_t *pEvent, size_t EventLen)
{
    ...
    const size_t peerScaByte = HCI_COMMAND_COMPLETE_BASE_SIZE + 43U;

    if (EventLen <= peerScaByte || pEvent[0] != HCI_EVENT_COMMAND_COMPLETE ||
        ...
        case HCI_H4_PACKET_COMMAND:
        {
            ...
            if (PacketLen >= HCI_DISPATCH_COMMAND_HEADER_SIZE)
            {
                ...
                if (opcode == HCI_SDC_COMPAT_OPCODE_READ_CONN_ACCEPT_TIMEOUT ||
                    ...
                    opcode == HCI_SDC_COMPAT_OPCODE_LE_REQUEST_PEER_SCA)
                {
